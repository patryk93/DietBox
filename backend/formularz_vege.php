...
<html lang="pl">
    <head>
        <title>Dieta Wegetariańska</title>
        <meta charset="UTF-8">
...
//tworzymy uchwyt do bazy danych:
        ...
        if (filter_input(INPUT_GET, "submit")) {
            ...
            switch ($akcja) {
                case "Dodaj" : dodajdoBD_vege($bd);
                echo $bd->select("select Id, Rodzaj, Kalorycznosc, LiczbaPosilkow, DlugoscDiety, Platnosc from zamowienia", array("Id", "Rodzaj", "Kalorycznosc", "LiczbaPosilkow", "DlugoscDiety", "Platnosc"));
                    break;
                case "Pokaż" : echo $bd->select("select Id, Rodzaj, Kalorycznosc, LiczbaPosilkow, DlugoscDiety, Platnosc from zamowienia", array("Id", "Rodzaj", "Kalorycznosc", "LiczbaPosilkow", "DlugoscDiety", "Platnosc"));
                    break;
                case "Usuń" : 
                    $Id=$_GET['Id'];
                    usuwanie($bd, $Id);
